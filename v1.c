...
char cmd[250];//comanda trimisa de client catre procesul copil
...
int loginUser(){
	...
	//BD
   		...
   		if( rc ) {
			...
			//rc=sqlite3_exec(db,sqlCmd,callback,0,&zErrMsg);//zetcode parametrized queries    
			...
			if(sqlite3_total_changes(db)!=changes){
				...
				rc = sqlite3_open("users.db", &db);
   				...
   				if( rc ) {
					...
					if(rc!=SQLITE_OK){
						printf("Failed to execute statement: %s\n",sqlite3_errmsg(db));
					} else printf("Perechea username->password s-a inserat cu succes in tabela loggedIn.\n");
					sqlite3_close(db);
				}
			}
